...
dataroot='./defocuseddata'
file=os.listdir(dataroot)
datalist=[]
labellist=[]
for i,filename in enumerate(file):
    picroot=os.path.join(dataroot,filename)
    picfile=os.listdir(picroot)
    for ii,filenamee in enumerate(picfile):
        picdata=os.path.join(picroot,filenamee)

        input_dict = h5py.File(picdata)

        img = input_dict['s3']
        img32_real = cv2.resize(img['real'].astype(np.float32),[32,32])
        img32_imag = cv2.resize(img['imag'].astype(np.float32), [32, 32])
        img = img32_real + img32_imag * 1j
        # train_vessels = np.expand_dims(echo1, axis=0)
        # img = Image.open(picdata)
        # img = img.convert('RGB')
        # img = img.astype(np.float32)
        # img = np.resize(img,[32,32])
        # img = img.resize((32,32))
        #img = matplotlib.image.imread(picdata)
        # #sub_np1 = img.numpy()
        # sub_ts = torch.from_numpy(img)
        # a = torch.transpose(sub_ts, 0, 2)
        # a = torch.transpose(a, 1, 2)
        img = np.expand_dims(img, -1)
        datalist.append(img)
for x in range(8):
    labellist.extend([x]*31)


facedataset = self_Dataset(datalist,labellist)

class self_DataLoader(Dataset):
    def __init__(self, root, train=True, dataset=facedataset, seed=1, nway=5):
        ...
        #     tv.transforms.Normalize([0.5],[0.25])
        #     ])
    ...
    def load_data(self, root, train, dataset):
        ...
        # few_selected_label = [1,3,5,7]
        print('selected labeled', few_selected_label)

        full_data_dict = {}
        few_data_dict = {}

        d = dataset  #把list中的data放到同一个标签下

        for i, (data, label) in enumerate(d):

            data = self.transform(data)
            # data = torch.from_numpy(data.transpose((1, 2, 0)))
    ...
    def load_batch_data(self, train=True, batch_size=16, nway=10, num_shots=5):
        if train:
        ...
        for i in range(batch_size):
            ...
            for j, _class in enumerate(sampled_classes):
                if j == positive_class:
                    ...
                    one_hot[j] = 1.0
                    # one_hot = torch.complex(one_hot,one_hot)
    ...
    def load_tr_batch(self, batch_size=16, nway=5, num_shots=5):
        return self.load_batch_data(True, batch_size, nway, num_shots)

    def load_te_batch(self, batch_size=16, nway=5, num_shots=5):
...
if __name__ == '__main__':
    D = self_DataLoader('/home/dell/PycharmProjects/facegnn/data', True)

    [x, label_y, one_hot_y, class_y, xi, label_yi, one_hot_yi, class_yi] = \
        D.load_tr_batch(batch_size=16, nway=5, num_shots=30)
