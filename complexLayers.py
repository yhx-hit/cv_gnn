...
class ComplexMaxPool2d(Module):
    ...
    def forward(self,inputr,inputi):
        return complex_max_pool2d(inputr,inputi,kernel_size = self.kernel_size,
...
class ComplexBatchNorm2d(_ComplexBatchNorm):
    ...
    def forward(self, inputr, inputi):
        ...
        inputr = Rrr[None,:,None,None]*input.real+Rri[None,:,None,None]*input.imag
        inputi = Rii[None,:,None,None]*input.imag+Rri[None,:,None,None]*input.real
...
class ComplexBatchNorm1d(_ComplexBatchNorm):
    ...
    def forward(self, input):
        ...
        if self.training or (not self.training and not self.track_running_stats):            
            # calculate mean of real and imaginary part
            mean_r = input.real.mean(dim=[0,2]).type(torch.complex64)
            mean_i = input.imag.mean(dim=[0,2]).type(torch.complex64)
            mean = mean_r + 1j*mean_i
        else:
            mean = self.running_mean
        ...
        input = (Rrr[None,:,None]*input.real+Rri[None,:,None]*input.imag).type(torch.complex64) \
                + 1j*(Rii[None,:,None]*input.imag+Rri[None,:,None]*input.real).type(torch.complex64)

        if self.affine:
            input = (self.weight[None,:,0,None]*input.real+self.weight[None,:,2,None]*input.imag+\
                    self.bias[None,:,0,None]).type(torch.complex64) \
                    +1j*(self.weight[None,:,2,None]*input.real+self.weight[None,:,1,None]*input.imag+\
                    self.bias[None,:,1,None]).type(torch.complex64)
